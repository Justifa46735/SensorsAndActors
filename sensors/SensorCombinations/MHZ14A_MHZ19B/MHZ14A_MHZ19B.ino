/**
 * CO2-Messung mit Sensor Typ MHZ14A und MHZ19B
...
int ppm_19B    = 0.0;  // PPM from UART
int temperature_19B = 0.0;  // Temperatur from Uart
int ppm_pwm_19B    = 0.0;  // PPM from PWM
...
SoftwareSerial co2Serial_19B(14, 12); // define MH-Z19 RX TX

// Die PWM hängt an Pin 16

const int pwmpin = D0;
const int pwmpin_19B = D3;

// Der eingestellte Messbereich (0-5000ppm)

const int range = 5000;
const int range_19B = 2000;
...
void setup() {
  ...
  pinMode(pwmpin_19B, INPUT);
  
  co2Serial.begin(9600);
  co2Serial_19B.begin(9600);
...
void loop() {
  ...
  ppm_pwm_19B = readCO2PWM_19B();
  readSensor(&ppm, &temperature);
  readSensor_19B(&ppm_19B, &temperature_19B);
  publishData(MQTT_MHZ19B_TOPIC_LEVEL_1, MQTT_PPM_PWM, ppm_pwm_19B);
  publishData(MQTT_MHZ19B_TOPIC_LEVEL_1, MQTT_PPM, ppm_19B);
  publishData(MQTT_MHZ19B_TOPIC_LEVEL_1, MQTT_TEMPERATURE, temperature_19B);
...
int readCO2PWM_19B() {
  ...
  do {
    ...
    ppm_pwm = range_19B * pulsepercent;
...
void readSensor_19B(int *ppm, int *temperature){

  // Befehl zum Auslesen schreiben
  
  co2Serial_19B.write(mhzCmdReadPPM, 9);
  ...
  memset(mhzResp, 0, 9);
  while (co2Serial_19B.available() == 0) {
    delay(1000);
  }

  // Die Antwort wird in den Speicher eingelesen.
  
  co2Serial_19B.readBytes(mhzResp, 9);
